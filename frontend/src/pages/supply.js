...
function procurementActiveChips(params, overview) {
  ...
  const statuses = (params.get("status") || "").split(",").filter(Boolean);
  if (statuses.length) push("status", "Status", statuses.map((key) => localizeText(optionLabel(procurementStatuses, key))).join(", "));
...
function procurementFilterNote(params, overview) {
  ...
  const statuses = (params.get("status") || "").split(",").filter(Boolean);
  if (statuses.length) parts.push(`Status: ${statuses.map((key) => optionLabel(procurementStatuses, key)).join(", ")}`);
  ...
  app.innerHTML = `<div class="page ops-page procurements-page">
    ...
    <div class="page-header">
      <div class="page-title">
        <h1>Xaridlar</h1>
        <p>Barcha xarid jarayonlarini boshqarish va kuzatish</p>
      </div>
...
function procurementTransitionsHtml(procurement) {
  ...
  const buttons = [...moves]
    ...
    .map((move) => {
      ...
      const label = optionLabel(procurementStatuses, move.status) || move.label;
...
function bindSupplierInvoiceForm(invoice = null) {
  ...
  form.addEventListener("submit", async (event)=>{
    event.preventDefault();
    const payload = { supplier_id: Number(field(form,"supplier_id")), procurement_id: field(form,"procurement_id") ? Number(field(form,"procurement_id")) : null, ticket_id: field(form,"ticket_id") ? Number(field(form,"ticket_id")) : null, supplier_offer_id: field(form,"supplier_offer_id") ? Number(field(form,"supplier_offer_id")) : null, delivery_batch_id: field(form,"delivery_batch_id") ? Number(field(form,"delivery_batch_id")) : null, logistics_id: field(form,"logistics_id") ? Number(field(form,"logistics_id")) : null, invoice_number: field(form,"invoice_number"), invoice_date: field(form,"invoice_date"), due_date: field(form,"due_date"), invoice_type: field(form,"invoice_type"), status: field(form,"status"), currency: "UZS", notes: field(form,"notes"), items: collectSupplierFinanceItems(form) };
    try {
